...
class Api4Service {
  ...
  Future<List<dynamic>> fetchNotifs(int id) async {
    final response = await http.get(Uri.parse('$baseUrl/notifLocation/user/$id'));
  ...
  Future<dynamic> createNotif(Map<String, dynamic> data) async {
    final response = await http.post(
      Uri.parse('$baseUrl/notifLocation/add'),
  ...
  Future<dynamic> updateNotif(int id) async {
    final response = await http.post(
      Uri.parse('$baseUrl/notifLocation/update/$id'),
      headers: {'Content-Type': 'application/json'},
    );
  ...
  Future<dynamic> deleteNotif(int id) async {
    final response = await http.delete(Uri.parse('$baseUrl/notifLocation/delete/$id'));
